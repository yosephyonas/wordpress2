...
$term_data = mprm_get_term_menu_items(); ?>
	...
	<div class="<?php echo esc_attr( apply_filters('mprm-widget-items-wrapper-class', 'mprm-container mprm-widget-items mprm-view-' . $view . mprm_popular_theme_class()) ); ?>">
		...
		<div class="mprm-columns-count-<?php echo esc_attr( $col ) ?> <?php echo empty($data['term']) ? 'mprm-all-items' : '' ?>">
			...
			foreach ($term_data as $term => $data) {
				if (in_array($view, array('list', 'grid'))) {
					...
					if (!empty($data['term'])) { ?>
						<div class="<?php echo esc_attr( apply_filters('mprm-simple-view-column', 'mprm-simple-view-column') ); ?>">
